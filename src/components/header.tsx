...
export function Header() {
  ...
  return (
    <header className="w-full border-b bg-white/80 dark:bg-gray-900/80">
      ...
      {/* Мобильное меню */}
      <Dialog as="div" className="md:hidden z-50 relative" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        ...
        <Dialog.Panel className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-gray-900 p-6 shadow-lg">
          ...
          <nav className="space-y-4">
            <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <HomeIcon className="h-5 w-5" /> Главная
            </Link>
            <Link href="/#about" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <InformationCircleIcon className="h-5 w-5" /> О системе
            </Link>
            <Link href="/#contacts" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <PhoneIcon className="h-5 w-5" /> Контакты
            </Link>
            <Link href="/profile" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <UserIcon className="h-5 w-5" /> Профиль
            </Link>
            <Link href="/notifications" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
